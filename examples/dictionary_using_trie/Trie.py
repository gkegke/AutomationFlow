...
class DTrie:
    ...
    Contents
     ...
     TODOS
      ...
      - Low Priority
        - consider making iter vs recur option, to prevent
        hitting python recurion limit

    """

    def __init__(self, fpath=None, use_set=False):
        """
        ...
        use_set allows you to initialize a set which from then on saves all
    ...
    def shallow_prefix_search(self, prefix=None):
        """
        """
        ...
        first_10 = [w for _, w in zip(range(10), wf)]
        last_10 = [w for _, w in zip(range(10), wr)]

        result = list(set(first_10 + last_10))
        result.sort()
